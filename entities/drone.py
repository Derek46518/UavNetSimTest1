...
from entities.packet import DataPacket, RtsPacket, CtsPacket
...
class Drone:
    ...
    def __init__(self,
        ...
        elif config.ROUTING_PROTOCOL == "cr_qgeo":
            self.routing_protocol = CRQGeo(self.simulator, self)
        else:
                ...
                if flag:
                    ...
                    if max_sinr >= config.SNR_THRESHOLD:
                        ...
                        if pkd.get_current_ttl() < config.MAX_TTL:
                            ...
                            if isinstance(pkd, (RtsPacket, CtsPacket)):
                                # RTS/CTS is a pure MAC-layer mechanism, handled once for every routing protocol
                                yield self.env.process(self.mac_protocol.handle_control_packet(pkd, sender))
                            else:
