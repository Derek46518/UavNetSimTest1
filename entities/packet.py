...
class RtsPacket(Packet):
    """
    Request-to-Send control packet, used by the MAC layer to reserve the channel before a unicast data
    transmission. Carries a reference to the pending data packet so the receiver knows how long to reserve
    the channel for when it replies with a CTS.
    """
    ...
    def __init__(self,
                 ...
                 channel_id,
                 creation_time=None):
        super().__init__(rts_packet_id, rts_packet_length, creation_time, simulator, channel_id)
...
class CtsPacket(Packet):
    """
    Clear-to-Send control packet, replied by the intended receiver of an RTS. Its transmission (and the
    channel reservation that accompanies it) is what protects the receiver from hidden-terminal collisions
    during the upcoming data/ack exchange.
    """
    ...
    def __init__(self,
                 ...
                 channel_id,
                 creation_time=None):
        super().__init__(cts_packet_id, cts_packet_length, creation_time, simulator, channel_id)
