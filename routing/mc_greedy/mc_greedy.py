...
from routing.mc_greedy.mc_neighbor_table import MCNeighborTable
from routing.mc_greedy.mc_packet import MCHelloPacket
from utils import config


class MCGreedy:
    """
    Main procedure of MC-Greedy (Mobility- and Congestion-Aware Greedy) Forwarding

    MC-Greedy extends the plain Greedy forwarding approach: a next-hop candidate is only considered when it makes
    positive geographic progress towards the destination (same restriction as Greedy), but among the valid
    candidates, the one is chosen that maximizes a weighted combination of geographic progress, predicted
    mobility/link stability and reported queue congestion, instead of simply the one closest to the destination.
    All neighbor mobility/congestion information is learned from received MC-Greedy hello packets rather than
    read directly from other drones' live state, keeping the routing decision distributed.

    Attributes:
        simulator: the simulation platform that contains everything
        my_drone: the drone that installed the MC-Greedy routing
        ...
        neighbor_table: neighbor table of MC-Greedy routing
    ...
    def __init__(self, simulator, my_drone):
        ...
        self.neighbor_table = MCNeighborTable(self.simulator.env, my_drone)
    ...
    def broadcast_hello_packet(self, my_drone):
        ...
        hello_pkd = MCHelloPacket(src_drone=my_drone,
    ...
    def packet_reception(self, packet, src_drone_id):
        ...
        if isinstance(packet, MCHelloPacket):
