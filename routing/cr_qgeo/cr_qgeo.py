...
from routing.cr_qgeo.cr_qgeo_packet import CRQGeoHelloPacket, CRQGeoAckPacket
from routing.cr_qgeo.cr_qgeo_table import CRQGeoTable
...
class CRQGeo:
    """
    Main procedure of CR-QGeo (Congestion- and Reliability-Aware Q-Learning Geographic Routing)

    CR-QGeo is derived from QGeo and preserves its tabular Q-learning architecture, next-hop
    selection mechanism, exploration policy, ACK behavior and Q-table structure. Only two
    algorithmic changes are introduced relative to QGeo:

    1. Congestion-aware intermediate reward:
           reward = geographic_progress - CR_QGEO_CONGESTION_WEIGHT * queue_congestion

    2. Continuous mobility/reliability-aware discount factor:
           stability = clip(1 - future_distance / maximum_communication_range(), 0, 1)
           gamma = CR_QGEO_GAMMA_MIN + (CR_QGEO_GAMMA_MAX - CR_QGEO_GAMMA_MIN) * stability

    Attributes:
        simulator: the simulation platform that contains everything
        my_drone: the drone that installed the CR-QGeo
    ...
    def __init__(self, simulator, my_drone):
        ...
        self.table = CRQGeoTable(self.simulator.env, my_drone, self.rng_routing)
    ...
    def broadcast_hello_packet(self, my_drone):
        ...
        hello_pkd = CRQGeoHelloPacket(src_drone=my_drone,
    ...
    def packet_reception(self, packet, src_drone_id):
        ...
        if isinstance(packet, CRQGeoHelloPacket):
            ...
            if packet_copy.dst_drone.identifier == self.my_drone.identifier:
                ...
                ack_packet = CRQGeoAckPacket(src_drone=self.my_drone,
                                              dst_drone=src_drone,
                ...
                if self.my_drone.transmitting_queue.qsize() < self.my_drone.max_queue_size:
                    ...
                    if void_area_flag == 0:
                        # snapshot queue congestion BEFORE enqueueing the newly received packet
                        congestion = self.my_drone.transmitting_queue.qsize() / self.my_drone.max_queue_size
                        congestion = min(max(congestion, 0.0), 1.0)  # clamp to [0, 1]
                        ...
                        distance2 = util_function.euclidean_distance_3d(self.my_drone.coords,
                                                                        packet_copy.dst_drone.coords)
                        progress = (distance1 - distance2) / maximum_communication_range()

                        reward = progress - config.CR_QGEO_CONGESTION_WEIGHT * congestion
                        max_q = self.table.get_max_q_value(packet_copy.dst_drone.identifier)

                        logger.debug('CR-QGeo pkd: %s, prev_hop: %s, this_hop: %s, progress: %.4f, '
                                     'congestion: %.4f, reward: %.4f',
                                     packet_copy.packet_id, src_drone_id, self.my_drone.identifier,
                                     progress, congestion, reward)
                    else:
                        reward = self.r_min
                        max_q = 0
        ...
        elif isinstance(packet, CRQGeoAckPacket):
    ...
    def update_q_table(self, packet, next_hop_id, next_hop_coords, next_hop_velocity):
        ...
        # calculate the discounted factor: reuse QGeo's existing future-position prediction
        ...
        # continuous mobility/reliability-aware discount factor
        stability = 1 - future_distance / maximum_communication_range()
        stability = min(max(stability, 0.0), 1.0)  # clamp to [0, 1]

        gamma = config.CR_QGEO_GAMMA_MIN + (config.CR_QGEO_GAMMA_MAX - config.CR_QGEO_GAMMA_MIN) * stability

        old_q = self.table.q_table[next_hop_id][dst_drone.identifier]
        new_q = (1 - self.learning_rate) * old_q + self.learning_rate * (reward + gamma * (1 - f) * max_q)
        self.table.q_table[next_hop_id][dst_drone.identifier] = new_q

        logger.debug('CR-QGeo Q-update pkd: %s, next_hop: %s, dst: %s, future_distance: %.4f, '
                     'stability: %.4f, gamma: %.4f, old_q: %.4f, new_q: %.4f',
                     data_packet_acked.packet_id, next_hop_id, dst_drone.identifier,
                     future_distance, stability, gamma, old_q, new_q)
